...
int main (void) 
{
    ...
    if (pid == 0)
    {
        /* Open with flags to open it in read-write mode and create it if it does not exists. */
...
/* quotes.txt is created with the content written out from the program boba.c, AND the we have a standard output aswell. */